...
def extract_completed_lap_times(
    ...
    for lap, sub in work.groupby("lap_number"):
        ...
        meta["lap_time"] = lt
        # Note: lap_time_invalid is NOT used to drop laps — user wants invalidated
        # laps graded the same as clean ones when time/distance checks pass.
...
- Every issue includes **count, per-lap rate, % of laps, hot distance bins, confidence**
    ...
    if tier_shown:
        ...
        st.dataframe(fix_df, width="stretch", hide_index=True)
