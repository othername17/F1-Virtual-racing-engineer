...
def analyze_driver(df: pd.DataFrame, bin_m: float = 25.0) -> dict[str, Any]:
    ...
    if len(laps) < 1:
        empty["notes"] = ["No valid laps found."]
        return empty

    lap_times = []
    for lap in laps:
        lt = _lap_time_for_lap(d, lap)
        if lt is not None:
            lap_times.append((lap, lt))
    if not lap_times:
        empty["notes"] = ["No lap times available for driver analysis."]
        return empty
    ...
    # Other laps for comparison (exclude best)
    others = [lap for lap, _ in lap_times if lap != best_lap]
    if not others:
        # Self-compare: still report technique notes on best lap only
        cmp = ref
        comparing = "best lap only (single timed lap)"
    else:
        # Median speed profile of other laps
        cmp = d[d["lap_number"].isin(others)]
        comparing = f"other laps vs best L{best_lap:.0f}"
...
def session_grade(
    ...
    Same-track comparisons only; not an absolute world ranking.
    """
    d = df[df["valid_sample"]].copy() if "valid_sample" in df.columns else df.copy()
    components: dict[str, dict[str, Any]] = {}

    # --- Pace (40): how close average of other laps is to best ---
    laps = []
    if not d.empty and "lap_number" in d.columns:
        for lap in d["lap_number"].dropna().unique():
            if lap < 0:
                continue
            lt = _lap_time_for_lap(d, lap)
            if lt is not None:
    ...
    if len(laps) >= 2:
        best = min(laps)
        mean_other = float(np.mean([t for t in laps if t > best + 1e-6] or laps))
        # 0% loss → 100; 3%+ slower average → ~40
        pct_off = (mean_other - best) / best * 100.0
        pace_score = float(np.clip(100 - pct_off * 20, 25, 100))
        components["Pace"] = {
            "score": pace_score,
            "weight": 0.40,
            "detail": f"Avg vs best: +{pct_off:.2f}% (best {best:.3f}s)",
        }
    elif len(laps) == 1:
        components["Pace"] = {
            "score": 75.0,
            "weight": 0.40,
            "detail": "Only one timed lap — neutral pace score.",
        }
    else:
        components["Pace"] = {
            "score": 50.0,
            "weight": 0.40,
            "detail": "No lap times — pace unscored.",
        }

    # --- Consistency (20) ---
    if len(laps) >= 3:
        std = float(np.std(laps))
        # 0.2s std great, 1.5s+ poor
        cons = float(np.clip(100 - (std / 0.02), 20, 100))
        components["Consistency"] = {
            "score": cons,
            "weight": 0.20,
            "detail": f"Lap time σ ≈ {std:.3f}s over {len(laps)} laps",
        }
    elif len(laps) == 2:
        gap = abs(laps[0] - laps[1])
        cons = float(np.clip(100 - gap * 40, 30, 100))
        components["Consistency"] = {
            "score": cons,
            "weight": 0.20,
        ...
        components["Consistency"] = {
            "score": 70.0,
            "weight": 0.20,
            "detail": "Need 2+ laps for consistency.",
        }

    # --- Cleanliness (20): locks / spins per lap ---
    ...
    clean_score = float(np.clip(100 - epl * 25, 15, 100))
    components["Cleanliness"] = {
        "score": clean_score,
        "weight": 0.20,
        "detail": f"Lock/spin/entry-OS events per lap ≈ {epl:.2f}",
    }

    # --- Balance control (10) ---
    bal_ids = [i for i in summaries if i.issue_id.startswith("us_") or i.issue_id.startswith("os_")]
    bal_epl = sum(s.events_per_lap for s in bal_ids)
    bal_score = float(np.clip(100 - bal_epl * 12, 20, 100))
    components["Balance control"] = {
        "score": bal_score,
        "weight": 0.10,
        "detail": f"US/OS events per lap ≈ {bal_epl:.2f}",
    }

    # --- Tire window (10) ---
    tire = [s for s in summaries if s.issue_id in ("tires_cold", "tires_hot")]
    tire_epl = sum(s.events_per_lap for s in tire)
    tire_score = float(np.clip(100 - tire_epl * 15, 30, 100))
    components["Tires"] = {
        "score": tire_score,
        "weight": 0.10,
        "detail": f"Out-of-window tire events/lap ≈ {tire_epl:.2f}",
    }

    total = sum(c["score"] * c["weight"] for c in components.values())
    # Small bonus if little time loss vs best
    zones = driver.get("time_loss_zones") or []
    if zones:
        top_loss = zones[0]["time_loss_s"]
        if top_loss < 0.05:
    ...
    return {
        ...
        "disclaimer": (
            "Session-relative grade only (same file / track). "
            "Not comparable across games, tracks, or fuel loads."
        ),
    }
